...
* **媒体按键与音量提示：** 可选择接管 F10 到 F12 来控制默认输出设备，并显示可关闭的 Tahoe 风格或经典风格音量提示。如果其他应用已经显示音量变化，可以关闭 FineTune 的音量提示。所有写入都会走 FineTune 的音量管线，因此即使在 macOS 自身因为硬件滑块不可用而把媒体键灰掉的 USB 接口或 HDMI 输出上，按键依然有效
